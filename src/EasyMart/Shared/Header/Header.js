...
const Header = () => {
  ...
  const handleColorChange = (color, logo) => {
    ...
    localStorage.setItem(
      "starting",
      JSON.stringify({ ...getStarting, primaryColor: color, logo: logo })
    );
  ...
  displayProducts?.map((item) =>
    searchProductsBold.push(item?.name?.split(headerSearchText))
  );
  ...
  const highlightText = (text, search) => {
    ...
    return parts.map((part, index) =>
      part.toLowerCase() === search.toLowerCase() ? (
        <strong key={index} style={{ color: getStarting?.primaryColor }}>
          {part}
        </strong> // Highlight search term
      ) : (
                ...
                <Nav className='mx-auto'>
                  <form className='tmp10-header-form'>
                    ...
                    {headerSearchText.length > 0 && (
                      <div
                        className='tmp10-suggestSearch'
                        style={{
                          padding:
                            displayProducts?.length === 0 ? "0px" : "15px 20px",
                          display:
                            headerSuggestBox === false ? "block" : "none",
                        }}
                      >
                          ...
                          <div key={product._id}>
                            <Link to='/products'>
                              <button
                                ...
                                style={{
                                  display: "flex", // Flex layout to align items inline
                                  alignItems: "center",
                                  gap: "10px", // Adds space between image and text
                                }}
                              >
                                {/* Image and text inline */}
                                <img
                ...
                <Nav>
                  <Navbar.Brand>
                    <Nav className='tmp10-header-icon-container'>
                      ...
                      <Nav.Link href='callto: ++8809609414141' target='_blank'>
                        <div
                          className='tmp10-header-icon'
                          style={{
                            backgroundColor: `${getStarting?.primaryColor}`,
                          }}
                        >
          ...
          <Navbar collapseOnSelect expand='lg' variant='dark'>
            <Container>
              ...
              <Nav>
                ...
                <Nav.Link>
                  <Dropdown>
                    ...
                    <Dropdown.Menu>
                      ...
                      {websiteColor?.map((color, i) => (
                        <p
                          key={i}
                          onClick={() =>
                            handleColorChange(color.color, color.logo)
                          }
                ...
                <Nav.Link as={Link} to='/cart' style={{ margin: "0px 10px" }}>
                  <span className='tmp10-header-cart'>
