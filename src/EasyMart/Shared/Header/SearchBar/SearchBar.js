...
const SearchBar = ({searchBarShow, handleSearchBarClose, searchBox, ...props}) => {
    const {headerSearchText, setHeaderSearchText, headerSuggestBox, handleSuggestClick, setHeaderSuggestBox,  displayProducts,getStarting, handleSearchClick, handleSearch, categories, handleAllCategory, handleCategory, selectedCategory} = useAuth();
                ...
                <Offcanvas {...props} show={searchBarShow} onHide={handleSearchBarClose} style={{overflow: searchBox === false ? 'visible' : 'hidden'}}>
                    ...
                    <Offcanvas.Body>
                        <form className="tmp-searchbar">
                            <div>
                            <input value={headerSearchText} onChange={handleSearchChange} onKeyPress={handleSearchKeypress} type="text" className="tmp-searchbar-input" placeholder="Search......." style={{border: `2px solid ${getStarting.primaryColor}`}}/>
                            <span onClick={() => setHeaderSearchText('')} style={{position: 'absolute', right: '100px', top: '8px', color: `${getStarting?.primaryColor}`, cursor: 'pointer'}}>
                                <FontAwesomeIcon icon={faTimes}/>
                            </span>
                            {/* <span onClick={() => setHeaderSearchText('')}> */}
                                <Link to='/products' onClick={handleSearchBarClose}>
                                    <button onClick={()=> handleSearchClick(headerSearchText)} style={{backgroundColor: `${getStarting.primaryColor}`}}><FontAwesomeIcon icon={faSearch}/></button>
                                </Link>
                            {/* </span> */}
                            {   
                                headerSearchText.length > 0 ?
                                <div className="tmp-header-mobile-suggestSearch canvas-search" style={{padding: displayProducts.length === 0 ? '0px' : '15px 20px', display: headerSuggestBox === false ? 'block' : 'none'}}>
                                    {
                            ...
                            {/* <Link to='/products' onClick={() => setSearchText('')}> */}
                                ...
                                <div className='tmp-searchbar-category'>
                                    {
                                        categories.slice(0).reverse().map(unique => <div key={unique.name} onClick={() => setHeaderSearchText('')}>
                                            <Accordion>
