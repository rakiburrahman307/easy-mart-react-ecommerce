...
const ManageOrder = () => {
    ...
    useEffect(() => {
        const vendorsOrder = totalOrder.filter(data => data.vendors_name === vendorUser.vendors_name);
  ...
  const dashboardData = [
    ...
    { title: 'Pending', count : orders?.filter(order => order.order_status === 1).length,  update: 'Updated 2 minutes ago' },
    { title: 'Delivered', count : orders?.filter(order => order.order_status === 3).length,  update: 'Updated 2 minutes ago' },
]


    return (
        <div className="easy-mart-manage-orders-container">
            <h2 className="text-center py-1" style={{color: getStarting?.primaryColor}}>Manage Orders</h2>
            {
                vendorUser.vendor &&
            ...
            }
            {
                ...
                <Table hover responsive>
                    ...
                    {
                        orders?.slice(0).reverse().map((order, i) => <tbody key={i}>
                            <tr>
                                <td>#{order?.invoice}</td>
                                ...
                                <td className="status">{
                                    ...
                                    &nbsp; &nbsp;
                                    <select onChange={(e) => handleUpdateStatus(order._id, e.target.value)} style={{ border: `1px solid ${getStarting?.primaryColor}`}}>
                                ...
                                <td>
                                    <Button disabled={order._id ? false : true} onClick={() => handleOrderDetails(order._id)} style={{backgroundColor: `${getStarting?.primaryColor}`, border: 'none'}}><FontAwesomeIcon icon={faEye} /></Button>
                                </td>
                            </tr>
                        </tbody>
                    )}
