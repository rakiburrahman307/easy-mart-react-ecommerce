...
useEffect( () => {
    ...
    .then(data => {
        ...
        for (const categories of data) {
            categories?.products?.map(data =>  newArray.push( {...data, cate_name: categories?.cate_name}))
...
useEffect(() => {
    ...
    if (superOfferCheckbox) {
        filteredProducts = filteredProducts.filter(product => product.superOffer);
    }
...
const showData = searchShowData =>{
    const matchedProducts = products?.filter(product => product.name.toLowerCase().includes(searchShowData.toLowerCase()));
...
const handleAddToCart = ( product, colorId ) => {
    ...
    if(existingIndex >= 0){
        newCart[existingIndex].quantity = newCart[existingIndex].quantity + newQuantity;
...
const handleProductOrders = () => {
    ...
    .then(result => {
        ...
        const filterUser = result?.filter(data => data.receiver_email === user.email)
...
useEffect( () => {
    ...
    return {
            getStarting, handleClearAllProductsPage, handleClearWithoutSearch, handleClearWithoutSuperOffer,
            handleClearWithoutCategories, handleClearWithoutBrands, handleClearWithoutVendors,
            products, setProducts, displayProducts, setDisplayProducts,
            superOffer, superOfferCheckbox, setSuperOfferCheckbox,
            categoriesProducts,
            categories, setCategories, handleAllCategory,
            handleCategory, selectedCategory, setSelectedCategory, handleCategoriesCheckbox, categoriesCheckbox, setCategoriesCheckbox,
            brands, handleBrands, selectedBrands, setSelectedBrands, handleBrandsCheckbox, brandsCheckbox, setBrandsCheckbox, 
            vendors, setVendors, vendorsCheckbox, setVendorsCheckbox, handleVendorsCheckbox,
            rangePrice, setRangePrice,
            cart, setCart, newQuantity, setNewQuantity, handleAddToCart, handleMinusToCart, handleRemove,
            rangePrice, setRangePrice, handleProductColor, productColorId,
            totalOrder, setTotalOrder, userOrder, setUserOrder, handleProductOrders,
            headerSearchText, setHeaderSearchText, headerSuggestBox, setHeaderSuggestBox, handleSearch, handleSearchClick, handleSuggestClick,
            user, userList, setUserList, admin, vendorUser, token, isLoading, authError, registerUser, loginUser, signInWithGoogle, setUpRecaptcha, logOut,
            messages, setMessages,
            handleAllBrands, handleAllVendors
    }
}
