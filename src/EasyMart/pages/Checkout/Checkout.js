...
const Checkout = () => {
    ...
    const { cart, setCart, user, getStarting, totalOrder, userOrder, setUserOrder, handleProductOrders, vendorUser} = useAuth();
    ...
    return (
        <>
           ...
           <Container>
                    ...
                    <div className="tmp-checkout-payment-container">
                        <div className="checkout-top-toggle-section">
                            <div className="checkout-top-toggle">
                                <div>
                                    <h6 style={{backgroundColor: (pageNumber === 1 | pageNumber === 2 | pageNumber === 3) ? getStarting?.primaryColor : "#00000061"}}>{pageNumber === 1 && "1"}{pageNumber > 1 && "✔"}</h6>
                                </div>
                            ...
                            <div className="checkout-top-toggle">
                                <div>
                                    <h6 style={{backgroundColor: (pageNumber === 2 | pageNumber === 3 )? getStarting?.primaryColor : "#00000061"}}>{pageNumber < 3 && "2"}{pageNumber > 2 && "✔"}</h6>
                                </div>
                            ...
                            <div className="checkout-top-toggle">
                                <div>
                                    <h6 style={{backgroundColor: pageNumber === 3 ? getStarting?.primaryColor : "#00000061"}}>3</h6>
                                </div>
                                <p>Payment details</p>
                            </div>
                        </div>
                        ...
                        }
                        {
                            pageNumber === 2 && (
                                <div className="tmp-checkout-shipping-container">
                                    ...
                                    <input placeholder="Receiver Name *" defaultValue={user?.displayName} {...register('receiver_name', { required: true })} style={{borderColor: getStarting?.primaryColor}}/>
                                    <input placeholder="Receiver Number *"  defaultValue={user?.email} {...register("receiver_email", { required: true })} style={{borderColor: getStarting?.primaryColor}}/>
                                    <input placeholder="Receiver Number *"  defaultValue={user?.phoneNumber} {...register("receiver_phone", { required: true })} style={{borderColor: getStarting?.primaryColor}}/>
                                    <input placeholder="Receiver Address *" {...register("receiver_location", { required: true })} style={{borderColor: getStarting?.primaryColor}}/>
                                    <textarea placeholder="Orders notes (Optional)" {...register("others", { required: true })} style={{height: '150px', borderColor: getStarting?.primaryColor}}/>
                                    {/* <input type="submit" value="PLACE ORDER" style={{backgroundColor: `${getStarting?.primaryColor || '#157ed2'}`, border: 'none'}}/> */}
                                </div>
                            )
                         }
                        {
                            pageNumber === 3 && <PaymentMethod paymentMethod={paymentMethod} setPaymentMethod={setPaymentMethod}/>
                        }
                        ...
                        <div className="checkout-next-btn mb-3">
                            ...
                            <Button
                                ...
                                style={{backgroundColor: getStarting?.primaryColor, border: 'none'}}
                            >
                ...
                {
                    pageNumber === 3 &&  <div className="text-center mb-4">
                        <label htmlFor="">
                            With Payment gateway
                        </label> &nbsp;
                            <button onClick={handleBkash} className="bkash-btn">
                                <img src="https://i.ibb.co/9YGfdHR/bikash.png" className="w-100" alt="bkash" />
                            </button>
                        </div>
                }
           </Container>
